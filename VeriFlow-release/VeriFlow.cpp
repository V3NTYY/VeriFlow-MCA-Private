...
static FILE* myTrace = NULL;
static bool damn = false;
...
int main(int argc, char** argv)
{
	...
	{
		mode = TEST_MODE;
                myFile = fopen("err_ec.txt","w");
                myTrace = fopen("ec_trace.txt","w");
		Test::test();
                fclose(myFile);
                fclose(myTrace);
		return EXIT_SUCCESS;
...
bool VeriFlow::verifyRule(const Rule& rule, int command, double& updateTime, double& packetClassSearchTime, double& graphBuildTime, double& queryTime, unsigned long& ecCount, FILE* fp)
{
	...
	for(unsigned int i = 0; i < vFinalPacketClasses.size(); i++)
	{
		...
		//fprintf(myTrace, "[%u:%u], %s\n", i, graph->totalRuleCount, packetClass.toString().c_str());
